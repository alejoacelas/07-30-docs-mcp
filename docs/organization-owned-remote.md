...
The source tree contains the secure broker: Claude receives MCP-specific tokens; Google grants are encrypted and remain inside the organization-operated service.

The public test endpoint is still in legacy token-passthrough mode. Do not add it as the organization’s production connector.

For an organizational rollout, replace the development ownership of every row:
